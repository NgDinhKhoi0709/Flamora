...
export default function OrderDetailPage({ params }: { params: { id: string } }) {
    ...
    const successImage = PlaceHolderImages.find(img => img.id === 'checkout-success');
                ...
                <Card className="max-w-4xl mx-auto">
                    {isSuccess && (
                         <CardHeader className="text-center items-center p-8 bg-green-50 rounded-t-lg">
                            ...
                            <CardTitle className="font-headline text-3xl text-green-800">Cảm ơn bạn đã đặt hàng!</CardTitle>
                            <p className="text-muted-foreground mt-2">Đơn hàng của bạn đã được nhận và đang được xử lý.</p>
                        </CardHeader>
                    )}
                    ...
                    <CardContent className="p-8">
                        <div className="grid md:grid-cols-2 gap-8">
                            <div>
                                <h3 className="font-semibold text-lg mb-4">Mã đơn hàng: <span className="font-mono text-primary">{order.id}</span></h3>
                                <div className="space-y-2 text-sm text-muted-foreground">
                                    <p><strong>Ngày đặt:</strong> {new Date(order.createdAt).toLocaleDateString('vi-VN')}</p>
                                    <p><strong>Tổng cộng:</strong> <span className="font-semibold text-foreground">{formatPrice(order.total)}</span></p>
                            ...
                            {order.items.map(item => (
                                <li key={item.id} className="flex py-4">
                                    <div className="h-20 w-20 flex-shrink-0 overflow-hidden rounded-md border">
                                        <Image src={item.productImage} alt={item.productName} width={80} height={80} className="h-full w-full object-cover" />
                                    </div>
